...
class NetworkHelper:
    ...
    def initializeInputs(self, input_filestring, N_repetitions=1):
        self.N_repetitions = N_repetitions # todo put this in the network config file instead or something
        ...
        # todo read in N_repetitions
        cell_inputs_ = np.tile(NetworkHelper.cell_inputs,(self.N_repetitions,1)) # concatenate repetitions in time
        NetworkHelper.cell_inputs = cell_inputs_ # should I clear cell_inputs_ now? gah I don't know how python mem management works
    ...
    def  simulateActivity(self, input_args, verboseplot=False):
        ...
        for i_rep in range(self.N_repetitions):
        ...
        # device.build(directory='output', compile=True, run=True, debug=False) # to c++ build
...
############################################################################################
        ...
        # BIN SIZE
        binwidth = 1 # ms
        numBins = self.N_repetitions * np.ceil((self.duration/ms) / binwidth)+1 #  https://docs.scipy.org/doc/scipy/reference/generated/scipy.ndimage.convolve1d.html
        ...
        raster_ = raster[self.N_input:,:] # just the non-input-receiving excitatory neurons
        avgRates = np.nansum(raster_,1) / (self.N_repetitions * self.duration/ms / 1000) # avg rates in Hz during the trial (remove ms units and convert to s)
        ...
        raster_smooth = filt.gaussian_filter1d(raster_,sigma=smoothSigma,axis=1) # todo should smooth BEFORE rebinning
        sumSmoothRate = np.sum(raster_smooth,0) # todo set this up relative to N_e

        meanSmoothRate = (1./(self.N_e - self.N_input))*sumSmoothRate # note, does the division make f.p. roundoff worse?

        IGNITION_THRESH = 0.5  # avg firing rate (Hz) among active cells -> to count as an ignition
        QUENCH_THRESH = 0.5
        PAROXYSM_THRESH = 25 # maximum allowed rate
        ...
        if do_score_rate:
            ...
            #    totalSpiking += (np.power(meanSmoothRate[idx], 2))  # sum square spiking including paroxysm periods
            for idx in stableBins:
                totalSpiking += np.power(meanSmoothRate[idx],2) # sum square spiking excluding paroxysm periods
...
###########################      package it all up
        ...
        NUM_COMPONENTS = 2  # 3  # temp - asynchrony and null # todo need to read this in automatically in the constructor
        score_components = np.zeros((NUM_COMPONENTS,))

        score_components[0] = rate_score
        score_components[1] = asynchrony_score

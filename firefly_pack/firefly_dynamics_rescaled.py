...
def firefly_dynamics_rescaled(oldPopulation, scores, alpha, beta, absorption, characteristicScales, MAXES, MINS):
    ...
    N_freeze = 5 #  - find this many 'best flies' for each pareto dimension and do not update their positions
    ...
    if (dominatedByOthers==0).all(): # i.e. there never was an update
        ...
        NUM_SPAWN = int(round(1.0*N_bugs / 10.0)) # respawn 10% of worst flies
        ...
        for i_score in range(np.shape(scores)[1]):
            ...
            bestScores = sorted(range(len(scores1D)), key=lambda i: scores1D[i])[-N_freeze:] # keep N_freeze top scores for each score dimension
            print " best scores = " + str(bestScores)
        ...
        paretoIDs = set(paretoIDs) # remove any repeat values
        #paretoIDs = list(paretoIDs) # flip back so can be used for indexing
    ...
    for i_dim in range(N_objectives):  # freeze best flies:
        for idx,val in enumerate(paretoIDs):
            newPopulation[:,idx] = oldPopulation[:,idx] # do not update the position of the winners
    # todo NewPopulation(freeze_idxs) = OldPopulation(freeze_idxs)

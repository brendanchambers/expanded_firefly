...
config_prefix = '2-28-2017 20x150 '  # for making the filestring - this should match the firefly config (todo make this automatic)
...
input_rate = 4 * Hz  # Hz # todo code clean get rid of this line
# input_current = 0 # nA

initial_Vm = "(-60 + 10 * (numpy.random.rand(1, self.N) - 0.5)) * mV"  # this is going to get read in as a string in the networkHelper class
        # todo this should be fixed along with the inputs in the input generation script
#
# ######## network parameters ######
C = 281 * pF
gL = 30 * nS
...
#p_connect_input = 0.2  # P(input cell -> excitatory cells) # we're going to assume this is fixed using p_connect_e
p_connect_ee = 0.2  # connection probabilities
#p_connect_ei = 0.4
#p_connect_ie = 0.2
#p_connect_ii = 0.3
...
logrand_sigma = 1.75  # would be interesting at some point to leave these out and examine this continuum computationally
...
tauw, a, b, Vr, EE, EI, taue, taui = 144 * ms, 4 * nS, 0.0805 * nA, -70.6 * mV, in_unit(0 * mV,mV), -75 * mV, 10 * ms, 3 * ms  # Regular spiking (as in the paper)
...
# save in json format
saveName = target_dir + config_prefix + ' networkconfig 2.json'
...
# scratch, testing recovery of the physical units outside the config file:
'''
